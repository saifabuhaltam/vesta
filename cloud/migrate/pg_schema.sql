...
-- Source: a fresh database built from db.py's SCHEMA, so every foreign key is
-- present. Regenerate the same way: create an empty install, point the generator
-- at its vesta.db. An already-migrated database reports columns added by ALTER
-- without their references, which would emit a schema missing those keys.
